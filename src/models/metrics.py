...
def regression_metrics(y_true, y_pred) -> dict:
    ...
    return {
        ...
        # Reliability: worst-case + tail error.
...
def timed_predict(model, x):
    """Return (predictions, latency_ms_per_1k_rows) — the 'response time' criterion."""
    t0 = time.perf_counter()
    preds = model.predict(x)
    dt = time.perf_counter() - t0
    per_1k = dt / max(len(x), 1) * 1000 * 1000  # ms per 1000 rows
    return preds, per_1k
